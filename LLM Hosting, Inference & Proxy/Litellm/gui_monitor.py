...
import threading
from inference_module import DB_PATH, get_metrics
        ...
        while self.running:
            try:
                ...
                if metrics['last_request']:
                    prompt, response, qt, rt, tt, rtime, model, provider = metrics['last_request']
                    ...
                    self.last_metrics.config(text=f"Metrics: {tt} tokens (Q:{qt}, R:{rt}), {rtime:.2f}s, {provider}/{model}")
